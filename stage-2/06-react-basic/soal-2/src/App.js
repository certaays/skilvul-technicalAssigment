...
class App extends Component {
  ...
  async fetchMovieList(){
    try{
      const anime = await fetch('https://api.jikan.moe/v3/top/anime')
      return(
        this.setState({movieList: await anime.json().top})
      )
    }catch(error){
      throw error
    }
  ...
  componentDidMount(){
    setInterval(() => this.fetchMovieList(), 1500)
  }
      ...
      <div className="container my-5">
        ...
        <div className="container my-5">
          <div id="daftar-anime" className="row">
              {
                this.state.isLoading ? animeList.map((movie) => 
