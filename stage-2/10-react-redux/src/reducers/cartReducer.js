...
const cartReducer = (state = initState, action) => {
  ...
  if (action.type === ADD_TO_CART) {
    ...
    let checkItem = state.addedItems.find(item => action.id === item.id)
      if(checkItem){
        addItem.quantity += 1
        return{
          ...state,
        ...
        addItem.quantity = 1
        let sum = state.total + addItem.price
  ...
  if (action.type === REMOVE_ITEM) {
    // Add code here
    let rmv = state.addedItems.find(item => action.id === item.id)
    let newItem = state.addedItems.filter(item => action.id !== item.id)
    let sum = state.total - (rmv.price*rmv.quantity)
